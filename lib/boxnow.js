...
async function getAccessToken() {
  // Checked here (not at module load) so a missing BoxNow env var only breaks
  // BoxNow delivery-request creation, not every module that imports this file —
  // notably the Stripe webhook route, where a module-load throw would have taken
  // down payment confirmation entirely for non-BoxNow orders too.
  if (!BOXNOW_API_URL || !CLIENT_ID || !CLIENT_SECRET) {
    throw new Error(
      "BoxNow misconfigured: BOXNOW_API_URL, BOXNOW_CLIENT_ID, and BOXNOW_CLIENT_SECRET are required."
    );
  }
...
export async function createDeliveryRequest(order) {
  ...
  console.log("[boxnow] createDeliveryRequest for order:", order.orderNumber, "→ locker:", destLocationId);
