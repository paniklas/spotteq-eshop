...
export function buildInvoicePayload(order) {
  ...
  // Allocated cumulatively rather than rounded per line and reconciled afterwards.
  //
  // Rounding each line independently and pushing the residue onto one line looks
  // equivalent and is not: when many lines each round UP (a small discount spread
  // over several lines, where every share sits just above half a cent) the rounded
  // shares overshoot, the residue is negative, and the correcting line ends up with
  // a NEGATIVE discount — which reads as a surcharge on a tax document. The totals
  // still reconcile in that case, so the guard below cannot catch it.
  //
  // Rounding the RUNNING TOTAL instead makes each share the difference between two
  // non-decreasing rounded cumulatives, so no share can be negative, and the last
  // one closes on `discount` exactly.
  const discounts = [];
  let runningGross = 0;
  let allocated = 0;
  for (const l of goods) {
    runningGross += l.gross;
    const target = subtotal > 0 ? round2((discount * runningGross) / subtotal) : 0;
    discounts.push(round2(target - allocated));
    allocated = target;
  }
