        ...
        >
            ...
            <div className="lg:hidden max-w-480 mx-auto page-x pb-16" style={fade(80)}>
                ...
                <div className="flex flex-col gap-3 mt-8">
                    <Link
                        ...
                        onClick={isOnClose}
                        tabIndex={isOpen ? 0 : -1}
                        className="h-12 flex items-center justify-center bg-black-custom text-white-custom rounded-[15px] font-aeonik text-[14px] tracking-wide"
                    >
                        SHOP ALL
                    </Link>
                    <Link
                ...
                {/* Language */}
                <div className="flex items-center gap-3 mt-10 font-tt text-[14px]">
                    ...
                    <LanguageOptions isMenuOpen={isOpen} onClose={isOnClose} className="uppercase" />
                </div>
            </div>

            {/* ─────────────── Desktop mega-menu (lg and up) ─────────────── */}
            <div className="hidden lg:flex max-w-480 mx-auto page-x gap-12 py-10 xl:py-32">
                {/* ── Left: nav content ── */}
            ...
            <div
                ...
                <div className="flex items-center gap-3 font-tt text-[12px] xl:text-[16px]">
                    <LanguageOptions isMenuOpen={isOpen} onClose={isOnClose} />
...
function MobileAccordion({ title, links, isMenuOpen, onClose }) {
    ...
    return (
        <div className="border-b border-black/15">
            <button
                ...
                aria-expanded={open}
                className="w-full flex items-center justify-between py-2 font-aeonik text-[20px] text-black-custom cursor-pointer"
            >
                {title}
                <svg
                    ...
                    <div className="flex flex-col gap-2 pb-4 pl-1">
                        {links.map(({ label, href }) => (
                            <Link
                                ...
                                href={href}
                                onClick={onClose}
                                tabIndex={isMenuOpen && open ? 0 : -1}
                                className="font-aeonik text-[16px] text-black-custom hover:opacity-60 transition-opacity"
                            >
                                {label}
                            </Link>
                        ))}
...
function LanguageOptions({ isMenuOpen, onClose, className = "" }) {
    const { locale, switchLocale, isPending } = useLocaleSwitch();

    return LANGUAGES.map(({ locale: optionLocale, label }) => {
        ...
        return (
            <button
                ...
                tabIndex={isMenuOpen ? 0 : -1}
                onClick={() => {
                    switchLocale(optionLocale);
                    onClose();
                }}
                className={`${className} transition-colors ${
                    isActive
                        ? "text-black-custom font-medium"
                        : `text-gray-language hover:text-foreground cursor-pointer ${isPending ? "opacity-50" : ""}`
                }`}
            >
                {label}
            </button>
