import Image from "next/image";
import { Link } from "@/i18n/navigation";
import { urlFor } from "@/sanity/lib/image";
import { getCategoriesByGroup } from "@/sanity/getData/getAllcategories";

const ShopBySeries = async ({ locale }) => {
    const series = await getCategoriesByGroup(locale, 'series')

    if (!series?.length) return null

    return (
        <section
            id="shop-by-series-section"
            className="w-full bg-white-custom py-10 xl:py-14"
        >
            <div className="max-w-480 mx-auto page-x">
                <h2 className="font-aeonik text-black text-[28px] xl:text-[35px] leading-[1.45] mb-8">
                    Shop by series
                </h2>

                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    {series.map((s) => (
                        <Link
                            key={s._id}
                            href={`/shop/category/${s.slug}`}
                            className="group relative z-20 block"
                        >
                            <div
                                className="relative overflow-hidden rounded-[25px]"
                                style={{ aspectRatio: "787/518" }}
                            >
                                {s.image && (
                                    <Image
                                        src={urlFor(s.image).width(800).url()}
                                        alt={s.title}
                                        fill
                                        sizes="(min-width: 1536px) 800px, (min-width: 768px) 50vw, 100vw"
                                        quality={100}
                                        className="object-cover rounded-[25px]"
                                    />
                                )}

                                <div className="absolute inset-0 p-4 xl:p-8 flex flex-col justify-between">
                                    {s.displayNumber && (
                                        <span className="font-tt font-semibold text-white-custom text-[16px] leading-[1.45]">
                                            {s.displayNumber}
                                        </span>
                                    )}

                                    <div className="flex flex-col gap-3 md:translate-y-[calc(1.6em+0.75rem)] md:group-hover:translate-y-0 transition-transform duration-500 ease-out">
                                        <span className="font-aeonik text-white-custom text-[20px] xl:text-[24px] leading-[1.6] relative w-fit">
                                            {s.title}
                                            <span className="absolute bottom-0 left-0 h-px w-0 bg-white-custom group-hover:w-full transition-all duration-500 ease-out" />
                                        </span>
                                        {s.description && (
                                            <p className="hidden md:block font-aeonik text-white-custom text-[14px] xl:text-[18px] max-w-120 opacity-0 group-hover:opacity-100 transition-opacity duration-500 delay-150">
                                                {s.description}
                                            </p>
                                        )}
                                    </div>

                                    <svg width="55" height="15" viewBox="0 0 55 15" fill="none" xmlns="http://www.w3.org/2000/svg" className="absolute bottom-3 right-4 w-9 h-9 md:w-14 md:h-14 opacity-100 translate-x-0 md:opacity-0 md:-translate-x-14 md:group-hover:opacity-100 md:group-hover:translate-x-0 transition-all duration-500 ease-out delay-100">
                                        <path d="M53.9856 8.07088C54.3762 7.68035 54.3762 7.04719 53.9856 6.65666L47.6217 0.292702C47.2312 -0.0978227 46.598 -0.0978227 46.2075 0.292702C45.8169 0.683226 45.8169 1.31639 46.2075 1.70692L51.8643 7.36377L46.2075 13.0206C45.8169 13.4111 45.8169 14.0443 46.2075 14.4348C46.598 14.8254 47.2312 14.8254 47.6217 14.4348L53.9856 8.07088ZM0 7.36377V8.36377H53.2785V7.36377V6.36377H0V7.36377Z" fill="white"/>
                                    </svg>
                                </div>
                            </div>

                            {/* Mobile only — description shown below the card in black */}
                            {s.description && (
                                <p className="md:hidden mt-3 font-aeonik text-black-custom text-[13px] leading-[1.3]">
                                    {s.description}
                                </p>
                            )}
                        </Link>
                    ))}
                </div>
            </div>
        </section>
    )
}

export default ShopBySeries
