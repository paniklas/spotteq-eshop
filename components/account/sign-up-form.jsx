            ...
            {step === 'start' && (
                <>
                    ...
                    <form onSubmit={handleStart} className="flex flex-col gap-4">
                        <div className="grid grid-cols-2 gap-3">
                            <div className="flex flex-col gap-1.5">
                                <label className="font-aeonik text-[13px] text-black-custom">{t('firstName')}</label>
                                <input type="text" required autoComplete="given-name" value={firstName} onChange={e => setFirstName(e.target.value)} className={INPUT} />
                            </div>
                            <div className="flex flex-col gap-1.5">
                                <label className="font-aeonik text-[13px] text-black-custom">{t('lastName')}</label>
                                <input type="text" autoComplete="family-name" value={lastName} onChange={e => setLastName(e.target.value)} className={INPUT} />
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input type="password" required autoComplete="new-password" value={password} onChange={e => setPassword(e.target.value)} className={INPUT} />
                        </div>
                    ...
                    <p className="font-aeonik text-[13px] text-gray-text text-center">
                        {t('haveAccount')}{' '}
                        <Link href="/sign-in" className="text-black-custom underline hover:text-black-custom/70">{t('signInLink')}</Link>
                    </p>
                </>
            )}
                ...
                <>
                    ...
                    <form onSubmit={handleVerify} className="flex flex-col gap-4">
                        <div className="flex flex-col gap-1.5">
                            ...
                            <input type="text" required autoComplete="one-time-code" autoFocus value={code} onChange={e => setCode(e.target.value)} className={INPUT} />
                        </div>
                        {error && <p className="font-aeonik text-[12px] text-orange-accent">{error}</p>}
                        <button type="submit" disabled={isLoading} className={`${BTN} bg-black-custom text-white-custom hover:bg-black-custom/90`}>
                            {isLoading ? <Spinner /> : t('verifyAndContinue')}
                        </button>
                    </form>

                    <button type="button" onClick={handleResend} className={LINK_BTN}>
                        {t('resendCode')}
                    </button>
                </>
            )}
